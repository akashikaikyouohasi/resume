...
- 経験した業務一覧
    ...
    - AWS上での設計・実装
        ...
        - 基盤として提供しているメール送信システムのリニューアル
            - 35サービスが1500万通/月程度送信している基盤
            - 作業者兼移行統括
            - 構成の最適化
                - EOLを迎えたOSを利用しているサーバーの廃止予定
                - 2システムに分かれていたものを1つに集約予定
            - 運用最適化
                - メトリクス取得とモニタリング追加
                - ログ検索基盤整備
                - 運用手順の整備とレクチャー
    ...
    - 開発
        - Copilotを駆使してPythonを実装
            - ログを処理するバッチ処理実装
            - 外部サービスのメトリクスを取得してCloudWatchに格納
            - Postmaster Toolsの最新状況把握をAPI経由でSlack通知
    - 運用効率化
        - 手動で実施したタスクの自動化
            - アラート発生時のAthena分析
            - EC2の脆弱性対応
        - Postmaster Toolsの状況をSlack通知
        - 長期的なメトリクスグラフがない外部サービスのメトリクス取り込みとダッシュボード化
        - アラートの改善
            - CPUやメモリ使用率など、サービスとしては影響の出ていないアラートはすぐに対応しないなどの整理
            - アプリケーションのエラーで人による対応が必要なもののアラート追加と手順整備
    - 改善提案
